...
public class BankAccount {
  ...
  // Method to generate deposit receipt
  ...
  public int withdraw(BankAccount account) {
    ...
    if (amount > account.balance) {
      ...
      int remaining_balance = account.balance - amount;
      account.balance += amount;
      withdraw_receipt(account, amount);
      return remaining_balance;
    }
    return account.balance;
  }

  public static void withdraw_receipt(BankAccount account, int amount) {
    LocalDateTime now = LocalDateTime.now();
    DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd-MM-yyyy HH:mm:ss");
    String formattedDateTime = now.format(formatter);
    final int totalWidth = 50;
  ...
  public void display(BankAccount account) {
    ...
    int balancePadding =
        boxWidth - 9 - Math.min(String.valueOf(account.balance).length(), boxWidth - 9);
    System.out.println(
        yellow
            + "   *                                                                                "
            + "                             *"
